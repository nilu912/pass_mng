...
(function (global, factory) {
    ...
    switch (typeof value) {
      ...
      case "object": {
        ...
        return (
          ...
          keys
            .map(
              (k) => `${stringify$1(k, seen)}: ${stringify$1(value[k], seen)}`
            )
            .join(", ") +
          " }"
        );
  ...
  class Signature {
    ...
    get s() {
      assertArgument(
        parseInt(this.#s.substring(0, 3)) < 8,
    ...
    [Symbol.for("nodejs.util.inspect.custom")]() {
      return `Signature { r: "${
        this.r
      }", s: "${this._s}"${this.isValid() ? "" : ', valid: "false"'}, yParity: ${this.yParity}, networkV: ${this.networkV} }`;
    }
    clone() {
  ...
  function getKzgLibrary(kzg) {
    const blobToKzgCommitment = (blob) => {
      if ("computeBlobProof" in kzg) {
        if (
          "blobToKzgCommitment" in kzg &&
          typeof kzg.blobToKzgCommitment === "function"
        ) {
    ...
    const computeBlobKzgProof = (blob, commitment) => {
      if (
        "computeBlobProof" in kzg &&
        typeof kzg.computeBlobProof === "function"
      ) {
        return getBytes(
          kzg.computeBlobProof(hexlify(blob), hexlify(commitment))
        );
      }
      if (
        "computeBlobKzgProof" in kzg &&
        typeof kzg.computeBlobKzgProof === "function"
      ) {
      ...
      if (
        ...
        return getBytes(
          kzg.computeBlobKZGProof(hexlify(blob), hexlify(commitment))
        );
      }
      assertArgument(false, "unsupported KZG library", "kzg", kzg);
    };
  ...
  function _parseEip7702(data) {
    const fields = decodeRlp(getBytes(data).slice(1));
    assertArgument(
      Array.isArray(fields) && (fields.length === 10 || fields.length === 13),
    ...
    const tx = {
      ...
      data: hexlify(fields[7]),
      accessList: handleAccessList(fields[8], "accessList"),
      authorizationList: handleAuthorizationList(
        fields[9],
        "authorizationList"
      ),
    };
  ...
  class Transaction {
    ...
    set authorizationList(auths) {
      this.#auths =
        auths == null ? null : auths.map((a) => authorizationify(a));
    }
  ...
  class TransactionResponse {
    ...
    constructor(tx, provider) {
      ...
      this.authorizationList =
        tx.authorizationList != null ? tx.authorizationList : null;
      this.#startBlock = -1;
    }
    toJSON() {
      const {
        ...
        authorizationList: allowNull(
          arrayOf((v) => {
            ...
            if (v.signature) {
              ...
              sig = Object.assign({}, v, { yParity: yParity });
            }
            return {
  ...
  class AbstractSigner {
    ...
    async populateAuthorization(_auth) {
      ...
      if (auth.chainId == null) {
        auth.chainId = (
          await checkProvider(this, "getNetwork").getNetwork()
        ).chainId;
      }
      if (auth.nonce == null) {
          ...
          async ({ provider, weight }, index) => {
            try {
              ...
              results[index] = Object.assign(
                normalizeResult(req.method, { result: result }),
                { weight: weight }
              );
            } catch (error) {
              results[index] = Object.assign(
                normalizeResult(req.method, { error: error }),
                { weight: weight }
              );
            }
          }
          ...
          if (found.length) {
            if (options && options.filter) {
              const filtered = options.filter(
                found.map((i) => Object.assign({}, i.info))
              );
              if (filtered == null) {
                resolve(null);
                ...
                if (match) {
                  ...
                  resolve(
                    new BrowserProvider(provider, undefined, {
                      providerInfo: info,
                    })
                  );
                } else {
              ...
              resolve(
                new BrowserProvider(provider, undefined, { providerInfo: info })
              );
            }
          } else {
            resolve(null);
